...
import java.util.stream.Stream;
 class Text_justification
 {
    ...
    public static List<String> fullJustify(String[] words, int maxWidth) {
        ...
        {
            int newlength=sumofwords+word.length()+currentwords.size();
            if(newlength> maxWidth)
            {
                int extrapad=maxWidth-(sumofwords+currentwords.size()-1);
                int roundrobinspace=Math.max(currentwords.size()-1,1);
                for(int i=0;i<extrapad;i++)
                {
                    currentwords.get(i%roundrobinspace).append(" ");
                }
        ...
        int extraspaces=maxWidth-(sumofwords+currentwords.size()-1);
        String space=Stream.generate(() -> " ").limit(extraspaces).collect(Collectors.joining(""));
